...
void CmEvaluation::PrintVector(FILE *f, const vecD &v, std::string name)
{
...
double CmEvaluation::Evaluate_(std::string gtImgW, std::string inDir, std::string resExt, vecD &precision, vecD &recall, vecD &tpr, vecD &fpr)
{
    vecS names;
    string truthDir, gtExt;
    int imgNum = 0;
    //CmFile::GetNamesNE(gtImgW, names, truthDir, gtExt);
...
void CmEvaluation::EvalueMask(CStr gtW, CStr &maskDir, vecS &des, CStr resFile, double betaSqr, bool alertNul, CStr suffix, CStr title)
{
    ...
    string gtDir, gtExt;
    int imgNum = 0;
    //CmFile::GetNamesNE(gtW, namesNS, gtDir, gtExt);
...
void CmEvaluation::MeanAbsoluteError(CStr inDir, CStr salDir, vecS des, bool zeroMapIfMissing)
{
    vecS namesNE;
    int imgNum = 0;
    //CmFile::GetNamesNE(inDir + "*.jpg", namesNE)
    int count = 0, methodNum = des.size();
    vecD costs(des.size());
    for (int i = 0; i < imgNum; i++){
