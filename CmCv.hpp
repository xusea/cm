...
struct CmCv {
    ...
    static inline cv::Size NormalizeSize(const cv::Size& sz, int shortLen, int unitLen = 1);
    ...
    static inline cv::Rect GetImgRange(cv::Point p1, cv::Point p2, cv::Size imgSz);
    
    // Check an image (with size imgSz) point and correct it if necessary
    static inline void CheckPoint(cv::Point &p, cv::Size imgSz);
    ...
    // Get mask region.
    static cv::Rect GetMaskRange(CMat &mask1u, int ext = 0, int thresh = 10);
    ...
    static int GetBorderPnts(cv::Size sz, double ratio, vector<cv::Point> &bdPnts);
    ...
    static void fillPoly(Mat& img, const vector<PointSeti> _pnts, const Scalar& color, int lineType = 8, int shift = 0, cv::Point offset = cv::Point());
    
    // down sample without convolution, similar to cv::pyrDown
    template<class T> static void PyrDownSample(CMat &src, Mat &dst);
    template<class T> static void PyrUpSample(CMat &src, Mat &dst, cv::Size dSz);
    ...
    static Mat getGrabMask(CMat &img3u, cv::Rect rect);//, CStr sameNameNE, int ext = 4
...
cv::Size CmCv::NormalizeSize(const cv::Size& sz, int shortLen, int unitLen)
{
    double ratio = double(shortLen) / min(sz.width, sz.height);
    return cv::Size(cvRound(sz.width * ratio / unitLen) * unitLen, cvRound(sz.height * ratio /unitLen) * unitLen);
}
...
void CmCv::CheckPoint(cv::Point &p, cv::Size imgSz)
{
...
cv::Rect CmCv::GetImgRange(cv::Point p1, cv::Point p2, cv::Size imgSz)
{
    CheckPoint(p1, imgSz);
    CheckPoint(p2, imgSz);
    return cv::Rect(min(p1.x, p2.x), min(p1.y, p2.y), abs(p1.x - p2.x), abs(p1.y - p2.y));
}
...
template<class T> void CmCv::PyrUpSample(CMat &src, Mat &dst, cv::Size sz)
{
