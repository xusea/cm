...
inline cv::Point round(const Point2d &p) { return cv::Point(cvRound(p.x), cvRound(p.y));}
...
//void splitStr(CStr& str, CStr& delimiters , vector<std::string>& tokens);
