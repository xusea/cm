...
//  testCM
//
//  Created by xusea on 16/1/16.
//  Copyright © 2016年 xusea. All rights reserved.
//

#include "CmSalCut.hpp"
...
Mat CmSalCut::CutObjs(CMat &_img3f, CMat &_sal1f, float t1, float t2, CMat &_border1u, int wkSize)
{
    ...
    if (border1u.data == NULL || border1u.size != _img3f.size){
        ...
        border1u = 255;
        border1u(Rect(bW, bH, _img3f.cols - 2*bW, _img3f.rows - 2*bH)) = 1;
        border1u(Rect(bW, bH, _img3f.cols - 2*bW, _img3f.rows - 2*bH)) -= 1;
    }
    Mat sal1f, wkMask;
    _sal1f.copyTo(sal1f);
...
int CmSalCut::Demo(std::string imgNameW, std::string gtImgW, std::string salDir)
{
    //if (argc != 5){
...
void f()
{
    //IplImage *mode = cvLoadImage("/Users/xusea/sketch2photo/imgdata/out_log/9.png", 1);
    IplImage *mode = cvLoadImage("/Users/xusea/sketch2photo/imgdata/dog2.png", 1);
    char buf[1001];
    char a[3][5];
    ...
    for (int i = 1; i < 31; i++)
    {
        sprintf(buf, "/Users/xusea/sketch2photo/imgdata/out_log/%d.png", i);
        //std::string filetest = std::string(buf);
        IplImage *test = cvLoadImage(buf, 1);
        //IplImage *test = cvLoadImage("C:\\Users\\xusea\\Desktop\\PointMatchDemo\\grayimg\\9.png", 1);
   ...
   /*     IplImage* canny_mode = cvCreateImage(cvGetSize(mode), mode->depth, 1);
    ...
    printf("%d ctime is %s\n",i, ctime(&timer));//得到日历时间
    //CmSalCut::Demo("/Users/xusea/sketch2photo/2.jpeg", "/Users/xusea/sketch2photo/2.png", "/Users/xusea/sketch2photo/3.png");
        char buf[1000];
        snprintf(buf, 1000, "/Users/xusea/sketch2photo/imgdata/%d.jpg",i);
        std::string inimg(buf);
        snprintf(buf, 1000, "/Users/xusea/sketch2photo/imgdata/out/%d.png", i);
        std::string outimg(buf);
        snprintf(buf, 1000, "/Users/xusea/sketch2photo/imgdata/out_log/%d.png", i);
        std::string logimg(buf);
        
        CmSalCut::Demo(inimg , outimg, logimg);
    timer = time(NULL);
    printf("ctime is %s\n",ctime(&timer));//得到日历时间
    }
     */
